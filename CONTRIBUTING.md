...
`type` の乗っ取り、ステータスバー、カーソル形状、日本語入力、`when` 句の効き方は、本物の
VS Code がないと確かめられません。項目は [test/manual/cases.ts](test/manual/cases.ts) に
書き出してあり、`-manual` を付けたときだけ、手順を1件ずつ表示して結果を尋ねます。
...
付けない場合は skip として一覧に出るだけなので、普段の `npm test` と CI は止まりません。
答え終わると、Pull Request の「手動で確認したこと」へそのまま貼れる形で結果が出ます。

この範囲に手を入れたときは、該当する項目を流してから送ってください。新しく手動でしか
確かめられないものを足した場合は、`cases.ts` に項目を1つ足します。そのとき
「なぜ自動テストにできないのか」も書いてください。自動化できるものが混ざると、
誰も実行しないまま古びていきます。
