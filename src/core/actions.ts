 ...
 * transaction, then `setMode`, then the cursor or selection. Positions carried by
 * the later actions are therefore in post-edit coordinates, and the cursor is
 * clamped under the mode the command ends in rather than the one it started in.
 */
export type Action =
