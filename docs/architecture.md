...
マークのガターアイコンは、編集のあと**記録している行番号へ描き直します**。VS Code は装飾を
周りの文字と一緒に動かしますが、マークは仕様として編集に追従しません。放っておくと両者が
食い違い、記録は3行目なのにアイコンは4行目、という状態になります。どちらが正しいにせよ両方は
描けないので、すべてのモーションが読むほう（保管庫）に合わせます。

装飾の `rangeBehavior` は `ClosedClosed` にしています。既定の `OpenOpen` は端に文字が入ると範囲を
広げるので、列0にある空の範囲がそこで行を開かれると2行にまたがり、ガターが両方に文字を描いて
いました（[#69](https://github.com/onzuka24/vscode-extension-v/issues/69)）。マークは点であって
範囲ではないので、広げないほうが正しい指定です。

ファイルツリーを開くときは、そこへ移動したうえで**選択だけを外します**。VS Code の reveal は
`setFocus` と `setSelection` の両方を呼ぶので、選択が残るとツリー上の印付け（`c`）と混ざり、
`d` が開いていたファイルまで消します。外せるのは `list.toggleSelection` だけです。`list.clear` は
選択が1つのとき `setFocus([])` も呼ぶため、移動した位置を捨ててしまいます。
